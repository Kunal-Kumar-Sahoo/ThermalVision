void PanelCallback(vp_channel event) { 
  switch (event) {
    ...
    case Button_5:  
      ...
      break; 
    case Button_6:
      SetInterpolate29Mode(); 
      break;
    case Button_7:  
      SetInterpolate64Mode();
      break;
    case Button_9:  
      SetScale(true,   1);
      break;
...
void TogglePower() {
   Power = !Power;
   if(Power) {
     Panel.send(Led_9, F("$RED"));
     statusCode = sensor.setMode(DEVICE_MODE::NORMAL);
     if(statusCode != 0) 
       Panel.sendf(MonitorLog,F("Mode change status %s"), sensor.getErrorDescription(statusCode)); 
...
void GraphClicked() { 
  ...
  Panel.send(GraphDrawCircle, F("$DEL")); 
  Panel.send(GraphDrawCircle, _Circle(clickx, clicky, 5));

  clickx = highByte(Panel.vpr_uint); 
  ...
  if(clickx >= hoffset && clicky >= voffset && clickx <=  hoffset + dispe && clicky <= voffset + dispe) { 
    clickx = (clickx - hoffset + ((ip * px) / 2)) / (ip * px); 
    clicky = (clicky - voffset + ((ip * px) / 2)) / (ip * px); 
    ...
    clickx = byte((clickx * ip * px) + hoffset); 
    clicky = byte((clicky * ip * px) + voffset);
    
    Panel.send(GraphDrawCircle, F("$WHITE")); 
    Panel.send(GraphDrawCircle, _Circle(clickx, clicky, 5));
...
void ToggleInfo() {
  InfoPanel = !InfoPanel; 
  Panel.send(Info, InfoPanel); 
}

void ToggleMirror() {
  mirror = !mirror;
  if(mirror) 
    Panel.send(Led_1, F("$GREEN")); 
  else 
...
void SetInterpolate29Mode() {
  ImageModeSelect = IP29; 
  Panel.send(Led_3, F("$OFF")); 
  Panel.send(Led_4, F("$YELLOW"));
  Panel.send(Led_5, F("$OFF"));
  if(Power)
    sensor.setFPSMode(FPS_MODE::FPS_1);
  hoffset = ((255 - (16 * 8)) / 2) + 8 + 35; 
  voffset = ((220 - (16 * 8)) / 2) + 12; 
  ScaleOffset = 30;
  TempScale(); 
  ip = 4;
  px = 4;
...
void TempScale() {
  ScaleStep = (ScaleTop - ScaleBottom) / 6; 
  
  Panel.send(Graph, F("$CLEAR")); 
  Panel.send(GraphDrawPixel, F("$16PX")); 
  
  Panel.send(GraphDrawPixel, F("$DBLUE")); 
  Panel.send(GraphDrawPixel, _Point(ScaleOffset,54)); 
 
  Panel.send(GraphText, _Point(ScaleOffset+10,70)); 
  Panel.sendf(GraphText, "%s°C", _FString(ScaleBottom + (ScaleStep * 0),3,1)); 

  Panel.send(GraphDrawPixel, F("$DPURPLE")); 
  Panel.send(GraphDrawPixel, _Point(ScaleOffset,70)); 
